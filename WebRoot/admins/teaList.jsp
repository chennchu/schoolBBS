...
<script type="text/javascript">
	...
	//init
	$(function() {
		var totalPage = "${requestScope.teacherCreteria.end}";
		var totalRecords = "${requestScope.teacherCreteria.total}";
		...
		<div class="row clearfix">
			...
			<div class="col-md-12 column">
				<ul class="nav nav-tabs">
					...
					<li class="active"><a href="<%=basePath%>teacher/teaListAll">老师信息管理</a></li>
				...
				<br>
				<form class="form-inline" action="<%=basePath%>teacher/listAllPost"
					method="post">
				...
				<table class="table">
					...
					<tbody>
						<c:forEach items="${requestScope.teachers }" varStatus="status"
							var="teachers">
							<tr>
								...
								<c:if test="${teachers.state eq 0 }">
									<td>正常</td>
									<td><a href="<%=basePath %>teacher/updateState?id=${teachers.id }&state=1">禁用</a> | <a href= "<%=basePath %>teacher/updateState?id=${teachers.id }&state=3">注销</a> | <a href="<%=basePath%>teacher/toUpdateTeacherPassword?id=${teachers.id}" >重置密码</a></td>
								</c:if>
								<c:if test="${teachers.state eq 1 }">
									<td>禁用</td>
									<td><a href="<%=basePath %>teacher/updateState?id=${teachers.id }&state=0">恢复</a> | <a href= "<%=basePath %>teacher/updateState?id=${teachers.id }&state=3">注销</a></td>
								</c:if>
								<c:if test="${teachers.state eq 3 }">
									<td>注销</td>
									<td>该用户已被注销,不能操作<td>
								</c:if>
							</tr>
